CREATE OR REPLACE FUNCTION dbms_final_projv1.status_trigger_function()
  RETURNS trigger AS
$BODY$
...
for rid,sid in select request_id, service_id from dbms_final_projv1.service_allocation where status='APPROVED'
...
update dbms_final_projv1.service_allocation set status ='UNAVAILABLE' where request_id = rid;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION dbms_final_projv1.status_trigger_function()
  OWNER TO "rootAWS";
  
  
CREATE TRIGGER onupdate
  AFTER UPDATE
  ON dbms_final_projv1.service_allocation
  FOR EACH STATEMENT
  EXECUTE PROCEDURE dbms_final_projv1.status_trigger_function();
